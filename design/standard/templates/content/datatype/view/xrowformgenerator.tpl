    ...
    {foreach $content.form_elements as $key => $item}
        <li class="xrow-form-element xrow-form-{$item.type} xrow-form-element-{$key}{if $item.error} xrow-form-error{/if}{cond( $item.class|ne(''), concat( ' ', $item.class ), '')}">
            ...
            {switch match=$item.type}
                {case match="checkbox"}
                    <label><input type="checkbox" name="XrowFormInput[{$id}][{$key}]" value="1" {if $item.def}checked="checked" {/if}/> &nbsp;{$item.name|wash}{if $item.req}<span title="Eingabe erforderlich">*</span>{/if}</label>
                    <div class="form-checkbox-padding">{cond( is_set( $item.desc ), $item.desc, '')}</div>
                {/case}
                {case match="text"}
                    <label>{$item.name|wash}{if $item.req}<span title="Eingabe erforderlich">*</span>{/if}</label>
