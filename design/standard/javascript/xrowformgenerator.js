...
/*var loop = function(node) {
    do {
        if(node.nodeType == 1) {
            window.console.log('nodeType');
            if(node.tagName == 'input' || node.tagName == 'textfield') {
...
function xrow_form_copy_element(button, el)
{
    var li = YAHOO.util.Dom.getAncestorByTagName(this, 'li');
    var ol = YAHOO.util.Dom.getAncestorByTagName(this, 'ol');
    if ( li && ol )
    {
        ...
        var temphtml = ''+ieInnerHTML(li);
        //loop(li);
        new_li.className = li.className;
        var find = ''+el.id+'\['+el.xrow_form_index+'\]',
            replace = ''+el.id+'['+xrow_form_index+']',
            findOption = ''+el.id+'_'+el.xrow_form_index,
            replaceOption = ''+el.id+'_'+xrow_form_index;
        temphtml = temphtml.replace(/find/g, replace);
alert(temphtml);
        temphtml = temphtml.replace( new RegExp(find, 'g'), replace);
        temphtml = temphtml.replace( new RegExp(findOption, 'g'), replaceOption);
        //window.console.log('xrow_form_index '+xrow_form_index);
        //window.console.log('el.xrow_form_index '+el.xrow_form_index);
        window.console.log(temphtml);
        new_li.innerHTML = temphtml;
        setEventListenerAndAddToOL(ol, new_li);
        xrow_form_index++;
    }
}*/
...
function setEventListenerAndAddToOL(ol_con, new_li, attribute_id) {
    ...
    //YAHOO.util.Event.addListener( YAHOO.util.Dom.getElementsByClassName( 'xrow-element-button-copy', 'img', new_li ), "click", xrow_form_copy_element, { id: attribute_id, xrow_form_index: xrow_form_index } );
